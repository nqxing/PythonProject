...
class zhihu(object):
    def __init__(self):
        ...
        self.label_ok = tkinter.Label(self.root, textvariable=self.ok_var, justify=CENTER ,font=("微软雅黑", 10, "bold"), fg='Firebrick')
        ...
        self.button_wtjc = tkinter.Button(self.root, text="检测", width=8, command=self.thread_wenti_jc, font=("微软雅黑", 12, "bold"))
        ...
        self.label_tips4 = tkinter.Label(self.root, text='3.更多好玩软件、资源欢迎关注微信公众号“最趣分享”')
    def gui_arrang(self):
    ...
    def get_offset(self):
        if self.id == None:
            ...
            for m in range(self.max):
                if m == 0:
                    self.ok_var.set('已完成 00%')
                offset = m * 20
                self.get_urls(offset, m + 1)
                if m != 0:
                    nums = str(m / self.max).split('.')
                    if len(nums[1]) > 2:
                        ok_num = nums[1][:2]
                        self.ok_var.set('已完成 {}%'.format(ok_num))
            time.sleep(2)
            while True:
                if len(threading.enumerate()) > 100:
                    time.sleep(5)
                elif len(threading.enumerate()) >= 3 and len(threading.enumerate()) < 100:
                    time.sleep(3)
                elif len(threading.enumerate()) < 3:
                    self.ok_var.set('已完成 100%')
            ...
            for data in datas:
                ...
                if img_urls:
                    ...
                    if len(img_urls) == len(img_names):
                        for i in range(len(img_urls)):
                            self.save_img(img_urls[i], img_names[i])
                if video_urls:
                    ...
                    if len(video_ids) == len(video_names):
                        for i in range(len(video_ids)):
                            self.save_video(video_ids[i], video_names[i])
        except:
            pass
            # print(traceback.format_exc())
