...
class getBase():
    '''
    获取基础所有学校 再获取每个学校的学生 老师 家长 信息写入excel
    '''
    def __init__(self):
        self.log_url = 'http://120.78.3.189:81/apps/base/user/ssoLogin'
        self.sch_url = 'http://120.78.3.189:81/apps/base/school/manager/list'
        self.stu_url = 'http://120.78.3.189:81/apps/base/school/student/list'
        self.tea_url = 'http://120.78.3.189:81/apps/base/school/teacher/list'
        self.par_url = 'http://120.78.3.189:81/apps/base/school/parent/list'
        self.headers = {
    ...
    def getSchList(self):
        ...
        with open("data\\学校信息.txt", "w", encoding='utf-8') as f:
            f.write(str(self.schDict))
        schIdList = list(self.schDict.keys())
        for s in range(len(schIdList)):
            self.book = xlwt.Workbook()  # 新建一个excel对象
            sId = schIdList[s]
            self.getStu(sId)
            self.getTea(sId)
            self.getPar(sId)
            self.book.save('data\\%s.xls' % self.schDict[sId][0])
            print('{}/{},{}获取成功~~'.format(s+1,len(schIdList),self.schDict[sId][0]))
            time.sleep(5)
        print('任务已全部完成~~')
    def getStu(self,sId):
        dict = {
