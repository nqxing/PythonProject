...
def savaSql():
    ...
    cursor = db.cursor()
    sql = 'INSERT INTO country_one(id, name, pid) VALUES(%s, %s, %s) '
    id = 0
    dirPath = 'D:\\PythonProject\\PersonalDemo\\pachong\\getCountry\\'
    f = open("{}全国地区信息.txt".format(dirPath), "r",encoding='utf-8')
    ...
    for i in range(len(shengNameList)):
        ...
        pid = 0
        try:
            cursor.execute(sql, (id, name, pid))
            db.commit()
            # 获取该省下的所有市名组成一个列表
    ...
    for shiId in shiIdList:
        # sql查询语句查询省id得出省名
        cx_sql = 'SELECT * FROM country_one WHERE id = %d' % shiId
        ...
        for shiName in shiNameList:
            id += 1
            try:
                cursor.execute(sql, (id, shiName, shiId))
                db.commit()
    ...
    for quId in quIdList:
        ...
        cx_sql = 'SELECT * FROM country_one WHERE id = %d' % quId
        cursor.execute(cx_sql)
        oneShi = cursor.fetchone()[1]
        cx_sql = 'SELECT * FROM country_one WHERE name = "%s"' % oneShi
        cursor.execute(cx_sql)
        cx_sql = 'SELECT * FROM country_one WHERE id = %d' % int(cursor.fetchone()[2])
        cursor.execute(cx_sql)
        ...
        for quName in quNameList:
            id += 1
            try:
                cursor.execute(sql, (id, quName, quId))
                db.commit()
    ...
    for zhenId in zhenIdList:
        ...
        cx_sql = 'SELECT * FROM country_one WHERE id = %d' % zhenId
        cursor.execute(cx_sql)
        tDic = cursor.fetchone()
        oneQu = tDic[1]
        tid = int(tDic[2])
        cx_sql = 'SELECT * FROM country_one WHERE id = %d' % tid
        cursor.execute(cx_sql)
        oneShi = cursor.fetchone()[1]
        cx_sql = 'SELECT * FROM country_one WHERE name = "%s"' % oneShi
        cursor.execute(cx_sql)
        cx_sql = 'SELECT * FROM country_one WHERE id = %s' % cursor.fetchone()[2]
        cursor.execute(cx_sql)
        oneSheng = cursor.fetchone()[1]
        ...
        for zhenName in zhenNameList:
            id += 1
            try:
                cursor.execute(sql, (id, zhenName, zhenId))
                db.commit()
    ...
    for pZhenId in pZhenIdList:
        ...
        for pZhenName in pZhenNameList:
            id += 1
            try:
                cursor.execute(sql, (id, pZhenName, pZhenId))
                db.commit()
                print('p---{}'.format(id))
            except:
                db.rollback()
    ...
    for cunId in cunIdList:
        ...
        for cunName in cunNameList:
            id += 1
            try:
                cursor.execute(sql, (id, cunName, cunId))
                db.commit()
                print(id)
            except:
                db.rollback()
