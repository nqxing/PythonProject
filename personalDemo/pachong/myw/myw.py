...
def download(title, durl, save_path):
    ...
    print('已下载{}部'.format(len(path_file_number)))

def get_durl(max):
    durls = []
    print('获取视频链接中，只下载评分高于70%的视频')
    for i in range(int(max)):
        url = 'http://www.xiaobi030.com/latest-updates/{}/'.format(i+1)
        headers = {
        ...
        for i, item in enumerate(items):
            ...
            # 比较大小
            if percent_to_int(positive) > percent_to_int("70%"):
...
def main():
    ...
    futures = []
    for i, durl in enumerate(durls):
        future = executor.submit(download, durl[0], durl[1], save_path)
