...
def main():
    ...
    for i in range(len(sheetList)):
        ...
        first_col8 = sheetList[i].col(7)  # xlwt中是行和列都是从0开始计算的
        ...
        for num in range(len(lists)):
            ...
            sheetList[i].write(new_row, 6, idCardNumber1, styleContent)
            sheetList[i].write(new_row, 7, createTime1, styleContent)
            # print('已写入', num + 1, '条数据~~')
        print(classNameList[i],'爬取成功~')
        time.sleep(2)
