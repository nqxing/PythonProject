...
class OnekeyInit:
    '''
    根据多学校平台学校现存信息，一键生成基础2.0各个模块模板
    '''
    def __init__(self):
        Stuxls = xlrd.open_workbook(r'基础模板\空\学生模板(空).xls', formatting_info=True)
        self.Stuxlsc = copy(Stuxls)
        Teaxls = xlrd.open_workbook(r'基础模板\空\教师模板(空).xls', formatting_info=True)
        self.Teaxlsc = copy(Teaxls)
        Graxls = xlrd.open_workbook(r'基础模板\空\年级模板(空).xls', formatting_info=True)
        self.Graxlsc = copy(Graxls)
        Claxls = xlrd.open_workbook(r'基础模板\空\班级模板(空).xls', formatting_info=True)
        self.Claxlsc = copy(Claxls)
        Semxls = xlrd.open_workbook(r'基础模板\空\学期模板(空).xls', formatting_info=True)
        self.Semxlsc = copy(Semxls)
        Secxls = xlrd.open_workbook(r'基础模板\空\作息时间模板(空).xls', formatting_info=True)
        self.Secxlsc = copy(Secxls)
        Buixls = xlrd.open_workbook(r'基础模板\空\建筑模板(空).xls', formatting_info=True)
        self.Buixlsc = copy(Buixls)
        Roomxls = xlrd.open_workbook(r'基础模板\空\教室模板(空).xls', formatting_info=True)
        self.Roomxlsc = copy(Roomxls)
        name = get_SchName()
        self.schPath = r"基础模板\%s" % name
        folder = os.path.exists(self.schPath)
        if not folder:  # 判断是否存在文件夹如果不存在则创建为文件夹
            os.makedirs(self.schPath)  # makedirs 创建文件时如果路径不存在会创建这个路径
    # 学生模板部分
    def StudentTemplate(self):
        ...
        if rows:
            for row in rows:
                ...
                lists.append(dic)
            sheet = self.Stuxlsc.get_sheet(0)
            ...
            self.Stuxlsc.save('%s\\学生模板.xls'%self.schPath)
    ...
    def TeacherTemplate(self):
        ...
        if rows:
            for row in rows:
                ...
                dic['educationNumber'] = educationNumber
                lists.append(dic)
            sheet = self.Teaxlsc.get_sheet(0)
            pin = Pinyin()
            ...
            self.Teaxlsc.save('%s\\教师模板.xls'%self.schPath)
    ...
    def GradeTemplate(self):
        ...
        if rows:
            for row in rows:
                ...
                dic['sorts'] = sorts
                lists.append(dic)
            sheet = self.Graxlsc.get_sheet(0)
            # i既是lis的下标，也代表每一列#处理表头
            for num in range(len(lists)):
                gradeName1 = lists[num]['gradeName']
            ...
            self.Graxlsc.save('%s\\年级模板.xls'%self.schPath)
    ...
    def ClassTemplate(self):
        ...
        if rows:
            for row in rows:
                ...
                dic['gradeName'] = gradeName
                lists.append(dic)
            sheet = self.Claxlsc.get_sheet(0)
            # i既是lis的下标，也代表每一列#处理表头
            for num in range(len(lists)):
                name1 = lists[num]['name']
                gradeName1 = lists[num]['gradeName']
            ...
            self.Claxlsc.save('%s\\班级模板.xls'%self.schPath)
    ...
    def SemesterTemplate(self):
        ...
        if rows:
            for row in rows:
                ...
                dic['status'] = status
                lists.append(dic)
            sheet = self.Semxlsc.get_sheet(0)
            # i既是lis的下标，也代表每一列#处理表头
            for num in range(len(lists)):
            ...
            self.Semxlsc.save('%s\\学期模板.xls'%self.schPath)
    ...
    def SectionTemplate(self):
        ...
        if rows:
            for row in rows:
                ...
                dic['endTime'] = endTime
                lists.append(dic)
            sheet = self.Secxlsc.get_sheet(0)
            # i既是lis的下标，也代表每一列#处理表头
            for num in range(len(lists)):
                no1 = lists[num]['no']
                name1 = lists[num]['name']
            ...
            self.Secxlsc.save('%s\\作息时间模板.xls'%self.schPath)
    ...
    def BuildingTemplate(self):
        ...
        if rows:
            for row in rows:
                ...
                lists.append(dic)
            sheet = self.Buixlsc.get_sheet(0)
            # i既是lis的下标，也代表每一列#处理表头
            for num in range(len(lists)):
            ...
            self.Buixlsc.save('%s\\建筑模板.xls'%self.schPath)
    ...
    def ClassroomTemplate(self):
        ...
        if rows:
            for row in rows:
                ...
                lists.append(dic)
            sheet = self.Roomxlsc.get_sheet(0)
            # i既是lis的下标，也代表每一列#处理表头
            for num in range(len(lists)):
            ...
            self.Roomxlsc.save('%s\\教室模板.xls'%self.schPath)
            print('教室数据写入成功,共', len(lists), '间教室~~')
        else:
            print('该校没有添加教室~')
if __name__ == '__main__':
    o = OnekeyInit()
    o.StudentTemplate() # 学生模板生成
    o.TeacherTemplate() # 教师模板生成
    o.GradeTemplate() # 年级模板生成
    o.ClassTemplate() # 班级模板生成
    o.SemesterTemplate() # 学期模板生成
    o.SectionTemplate() # 作息时间模板生成
    o.BuildingTemplate() # 建筑模板生成
    o.ClassroomTemplate() # 教室模板生成