...
from robot.models import pubEleSignVdtoken
    ...
    try:
        ...
        if r.status_code == 400 and r.json()['message'] == '账户存在风险,需要图形验证码':
            ...
            if r.status_code == 200 and 'validate_token' in r.json():
                result = {'status': 0, 'message': '验证码发送成功，请在5分钟内回复收到的验证码：\n\n注：若需重新发送手机号请先回复“饿了么签到”哦', 'validate_token': r.json()['validate_token'], 'mobile': mobile}
                return result
            else:
...
def captcha_yz(mobile, captcha_hash, captcha, openId):
    try:
        ...
        if r.status_code == 400 and r.json()['message'] == '图形验证码错误':
            ...
            if vd_values.exists():
                vd_value = vd_values[0]
                vd_value.mobile = mobile
                vd_value.vdtoken = r.json()['validate_token']
                vd_value.save()
            else:
                eleVdtoken = pubEleSignVdtoken()
                eleVdtoken.wx_open_id = openId
                eleVdtoken.mobile = mobile
                eleVdtoken.vdtoken = r.json()['validate_token']
                eleVdtoken.save()
            result = {'status': 0, 'message': '验证码发送成功，请在5分钟内回复收到的验证码：\n\n注：若需重新发送手机号请先回复“饿了么签到”哦'}
            return result
        else:
