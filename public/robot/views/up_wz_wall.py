        ...
        for s in skins:
            for k, p in enumerate(skin_names[s]):
                ...
                if values.exists():
                    ...
                    if s in bm_dict:
                        if '|' in bm_dict[s]:
                            bm = ' '.join(bm_dict[s].split('|')) + ' {}'.format(pf_name)
                        else:
                            bm = bm_dict[s] + ' {}'.format(pf_name)
                    else:
                        bm = pf_name
                    pub.hero_name_bm = bm
                    pub.save()
...
def run_main():
    ...
    if results:
        ...
        write_log(1, '王者壁纸更新成功，新增了{}张壁纸'.format(len(results)))
        ...
        write_log(1, '程序运行完毕，本次未新增王者荣耀壁纸'.format(len(results)))
