from robot.models import pubWZHeroName, pubWZwinRate, pubWZSkill, pubWZEquip, pubWZRune, pubWZKZ, pubWZIntroduce, pubWZZH
    ...
    try:
        ...
        for i in hero_ids:
            ...
            if values.exists():
                value = values[0]
                if i in bm_dict:
                    if bm_dict[i] != value.hero_name_bm:
                        value.hero_name_bm = bm_dict[i]
                        value.save()
            else:
                pub = pubWZHeroName()
                pub.hero_id = int(i)
        ...
        for i in hero_ids:
            ...
            if text != None:
                ...
                write_log(3, "{}".format(text))
                write_log(3, "获取英雄id[{},{}]信息出错".format(i, hero_dic[i]))
        # windows环境
