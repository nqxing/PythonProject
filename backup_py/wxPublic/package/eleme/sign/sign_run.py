...
def send_sign(openId, state):
    ...
    if state:
        cursor.execute(
            ''' SELECT sid, users_id FROM sign WHERE state = 'yes' AND is_bd = 'yes' ''')  # 查找饿了么库里的账号表，目前只取第一个账号
        values = cursor.fetchall()
    else:
        cursor.execute(
            ''' SELECT sid, users_id FROM sign WHERE open_id ="{}" '''.format(openId))  # 查找饿了么库里的账号表，目前只取第一个账号
        values = cursor.fetchall()
    if values:
        for v in values:
            sid, users_id = v[0], v[1]
            ...
            if type(results).__name__ == 'list':
                ...
                wx_str += result
                cursor.execute(
                    "INSERT INTO sign_info (open_id, info, sign_time) VALUES ('{}', '{}', '{}')".format(
                        openId, wx_str.strip(), datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
                conn.commit()
                with open("{}\{}.txt".format(ELMME_SIGN_TXT, openId), "a", encoding='gbk') as f:
                    f.write(wx_str.strip() + "\n-------------\n")
            else:
